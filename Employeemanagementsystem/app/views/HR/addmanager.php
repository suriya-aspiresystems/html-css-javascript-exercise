<?php
$validation = new addmanager();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Add Employee</title>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/js/bootstrap.bundle.min.js"></script>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/css/bootstrap.min.css">
    <link rel = "icon" href = "https://www.google.com/url?sa=i&url=https%3A%2F%2Ficonscout.com%2Ficon%2Fwork-1882975&psig=AOvVaw1o-fXzrpm1A8xe-ppCpoF1&ust=1665828729494000&source=images&cd=vfe&ved=0CA0QjRxqFwoTCIiT7PTA3_oCFQAAAAAdAAAAABAD"  type = "image/x-icon">
    <style>
        .errormessage
        {
            margin-left: 200px;
            text-align: center;
            padding: 2px;
            color: red;
            font-style: bold;
        }
        .error { color: red;}
    </style>
</head>
<body>
    <div class ="container my-5">
        <h2>Add Manager</h2>
        <form method ="POST">
             <div class="row mb-3 my-3">
                <label for="employeeid" class="col-sm-3 col-form-label">Manager ID</label>
                <div class="col-sm-6">
                    <input type="text" name="managerid" id="" class="form-control" value="" placeholder="Enter your Employee ID">
                </div>
            </div>  
            <div class="row mb-3 my-3">
                <label for="name" class="col-sm-3 col-form-label">Name</label>
                <div class="col-sm-6">
                    <input type="text" name="name" id="" class="form-control" value="" placeholder="Enter your name">
                </div>
            </div>
            <span class="error"><?php $validation->nameValid();?></span>
            <div class="errormessage" id="emailerr"></div>
            <div class="row mb-3">
                <label for="email" class="col-sm-3 col-form-label">Email</label>
                <div class="col-sm-6">
                    <input type="email" name="email" id="" class="form-control" value="" placeholder="Enter your Email">
                </div>
            </div>
            <span class="error"><?php $validation->emailvalid();?></span>
            <div class="errormessage" id="emailerr"></div>
            <div class="row mb-3">
                <label for="phone" class="col-sm-3 col-form-label">Phone Number</label>
                <div class="col-sm-6">
                    <input type="tel" name="phone" id="" class="form-control" value="" placeholder="Enter your Phone Number">
                </div>
            </div>
            <span class="error"><?php $validation->emailvalid();?></span>
            <div class="errormessage" id="emailerr"></div>
            <div class="row mb-3">
                <label for="department" class="col-sm-3 col-form-label">Department</label>
                <div class="col-sm-6">
                    <input type="text" name="department" id="" class="form-control" value="" placeholder="Enter your Department">
                </div>
            </div>
            <div class="row mb-3">
                <label for="status" class="col-sm-3 col-form-label">Status</label>
                <div class="col-sm-6">
                    <input type="text" name="status" id="status" class="form-control" value="">
                </div>
            </div>
          <div class = "row mb-3">
            <div class="offset-sm-3 col-sm-3 d-grid">
                <button class="btn btn-primary" name="sub_btn">Submit</button>
            </div>
            <div class="col-sm-3 d-grid">
                <a href="http://localhost/Employeemanagementsystem/public/manager" class="btn btn-outline-primary" role="button">Cancel</a>
            </div>
          </div>
            
        </form>
		<?php
		  $insertData = new Managerallcrud();
		  $insertData->insert();
		?>
    </div>
</body>
</html>